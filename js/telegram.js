...
export class TelegramNotifier {
  ...
  formatDailyReport(todayKey, streak, level, todayState, articleTitle) {
    ...
    if (todayState.task2Done) {
      msg += `✅ *任務 2：300字雙重朗讀*（《${articleTitle || '認知短文'}》盲讀+重音精讀）\n`;
    } else {
    ...
    if (todayState.task3Done && t3) {
      msg += `✅ *任務 3：3~5分鐘耐力心流隨心講*（${t3.overallScore || 92} 分）\n`;
    ...
    const coachPraise = (t1 && t1.coachPraise) || (t3 && t3.coachPraise) || '踏出開口表達的第一步就是100分！大腦語言迴路已激活！';
    msg += `\n💬 *教練寄語*：\n"${coachPraise}"\n\n`;
